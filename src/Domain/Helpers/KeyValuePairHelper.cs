...
namespace PilotLookUp.Domain.Helpers
{
    public class KeyValuePairHelper : PilotObjectHelper, IPilotObjectHelper
    {
        ...
        {
            _lookUpObject = keyValuePair.Value;
            _name = objectsRepository?.GetUserStates().FirstOrDefault(i => i.Id == keyValuePair.Key)?.Name ?? Resources.Strings.InvalidName;
            _isLookable = true;
        }
